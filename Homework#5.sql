...
ALTER TABLE room_stays ALTER COLUMN rate decimal(18,2);
ALTER TABLE room_stays ALTER COLUMN date datetime;

INSERT INTO room_stays(room_id,sale,guest_id,date,rate)
VALUES (1,100,3,getdate(),10),
	   (2,220,1,DATEADD(month,-1,getdate()),8),
	   (9,150,2,DATEADD(year,-1,getdate()),5),
	   (10,250,2,getdate(),10),
	   (4,200,1,getdate(),10);
...
SELECT users.user_name as 'User', roles.role_name 'Role', roles.role_description as 'Role Description' FROM Users  JOIN Roles ON users.role_id = roles.id;

/* #2 */ 

SELECT classes.name as 'Class', COUNT(*) as 'Count of Guests' FROM levels  RIGHT JOIN classes ON levels.classes_id = classes.ID GROUP BY classes.name;

/* #3 */

SELECT guest.name, classes.name, levels.Level, 
CASE WHEN levels.Level >= 1 AND levels.Level < 5 THEN 'beginner (lvl 1-5)' 
WHEN levels.Level >= 5 AND levels.Level < 10 THEN 'intermediate (lvl 5-10)' 
WHEN levels.Level >= 10 THEN 'expert (lvl 10+)' 
ELSE '' END AS 'Experience' 
FROM levels  RIGHT JOIN guest  ON levels.guest_id = guest.id LEFT JOIN classes ON levels.classes_id= classes.id order by guest.name asc;
...
AS   
RETURN(
	SELECT  rooms.rooms_name as 'Room Name', taverns.tavern_name as 'Tavern Name' FROM rooms  JOIN taverns  ON rooms.tavern_id = taverns.id
where rooms.id NOT IN (SELECT DISTINCT room_stays.room_id FROM room_stays where @ParticularDay BETWEEN room_stays.date AND room_stays.date + (sale/rate)));
GO
...
AS   
RETURN(
	SELECT DISTINCT rooms.rooms_name as 'Room Name', taverns.tavern_name as 'Tavern Name', room_stays.rate FROM Rooms  JOIN Taverns  ON rooms.tavern_id = taverns.id JOIN room_stays  ON room_stays.room_id = rooms.id
where rooms.id IN (SELECT DISTINCT room_stays.room_id FROM room_stays as s where room_stays.rate BETWEEN @MinPrice AND @MaxPrice));
GO
...
INSERT INTO rooms
(id, rooms_name, status_id, tavern_id)
VALUES
(70, 'Cheapest Room By A Penny', 1, 1)
GO


INSERT INTO room_stays
(id, sale, guest_id, room_id, date, rate)
VALUES
(70, 600, 3, 25, '2020-03-01', (SELECT MIN(Rate)-0.01 FROM openRooms(50,100)))
GO
